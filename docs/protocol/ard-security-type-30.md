...
- On RFB 3.8 failure, WinARD fully consumes the bounded reason payload. If either credential contains a non-ASCII UTF-8 byte, the raw reason is cleared without decoding and only the fixed generic rejection message is retained, avoiding normalization-equivalent disclosures. Otherwise WinARD replaces every exact username/password byte sequence with `[REDACTED]`, checks the redacted bytes again, decodes invalid UTF-8 with replacement characters into a clearable character buffer, escapes unsafe display-control runes into a second clearable buffer, and limits the final visible output to 4096 UTF-16 characters. Escaped `\uXXXX` text counts toward that limit. On RFB 3.3 and 3.7 failure it reads no reason bytes.
- Before creating the sole immutable reason string, WinARD checks the final sanitized character span against both ASCII credentials using ordinal matching. If byte redaction, control escaping, invalid UTF-8 replacement, or surrounding server bytes have recomposed either complete credential, the entire remote reason is discarded and only the fixed generic rejection message remains.
