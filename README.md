...
- 画质面板可在连接前选择自动、100%、75%、50% 或 25% 分辨率；会话中更改需要重新连接。
- 暂态网络中断可取消自动重连；全屏工具栏可由顶部热区或 `Ctrl+Alt+T` 唤回，`Esc` 优先关闭本地弹层再退出全屏。
- 全局设置支持主题、剪贴板默认值、诊断等级、保险库自动锁定和事务式凭据后端迁移。
...
Mac 端不需要安装 WinARD Helper、代理或虚拟显示器；只需启用 macOS 自带的“远程管理”（SSH 模式另需启用“远程登录”）。

## 画质、缩放与重连

百分比分辨率是连接启动配置，只在建立新连接时应用。自动档按连接开始时的带宽目标解析：不高于 1 MiB/s 为 25%，不高于 2 MiB/s 为 50%，不高于 4 MiB/s 为 75%，其余为 100%。当前会话中修改分辨率或色彩会显示“需要重新连接后生效”，可选择立即重新连接或稍后生效。

若目标 Mac 在首帧阶段拒绝首选画质配置，WinARD 最多新建一次安全连接，以 `100% + BGRA32 + Zlib-first` 回退。运行期暂态 EOF、网络错误或远端关闭会进入带倒计时的可取消自动重连；认证、权限、协议、安全类型和 SSH 主机密钥等确定性错误不会自动重试。

这些机制旨在平衡帧率、色彩、分辨率和带宽，但真实带宽改善仍是 `External gate`。在同一候选版本完成 macOS 26.5 对照测试前，不应把百分比档位描述为已达到任何带宽节省比例。
...
- 诊断采用有界结构化事件和白名单 ZIP；不包含主机名/IP。
- 诊断只导出稳定枚举、版本与关联 ID、受限聚合数值，以及安全错误代码和主机密钥指纹；不导出主机名/IP、远程画面、账户、路径、秘密、凭据键、原始异常正文或剪贴板正文。
- 凭据后端迁移先复制并验证，再以数据库事务更新引用；失败保留原引用和源秘密，提交后才可选择清理源副本。
- SQLite 设备库不保存明文密码。
